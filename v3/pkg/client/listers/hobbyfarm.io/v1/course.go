...
type CourseLister interface {
	...
	List(selector labels.Selector) (ret []*v12.Course, err error)
...
// List lists all Courses in the indexer.
func (s *courseLister) List(selector labels.Selector) (ret []*v12.Course, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v12.Course))
	})
	return ret, err
}
...
type CourseNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v12.Course, error)
...
// List lists all Courses in the indexer for a given namespace.
func (s courseNamespaceLister) List(selector labels.Selector) (ret []*v12.Course, err error) {
...
// Get retrieves the Course from the indexer for a given namespace and name.
func (s courseNamespaceLister) Get(name string) (*v12.Course, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v12.Resource("course"), name)
	}
	return obj.(*v12.Course), nil
}
