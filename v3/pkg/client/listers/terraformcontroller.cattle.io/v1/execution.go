...
type ExecutionLister interface {
	...
	List(selector labels.Selector) (ret []*v12.Execution, err error)
...
// List lists all Executions in the indexer.
func (s *executionLister) List(selector labels.Selector) (ret []*v12.Execution, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v12.Execution))
	})
	return ret, err
}
...
type ExecutionNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v12.Execution, error)
...
// List lists all Executions in the indexer for a given namespace.
func (s executionNamespaceLister) List(selector labels.Selector) (ret []*v12.Execution, err error) {
...
// Get retrieves the Execution from the indexer for a given namespace and name.
func (s executionNamespaceLister) Get(name string) (*v12.Execution, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v12.Resource("execution"), name)
	}
	return obj.(*v12.Execution), nil
}
