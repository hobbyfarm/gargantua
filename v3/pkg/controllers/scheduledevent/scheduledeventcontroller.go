...
func (s ScheduledEventController) provisionScheduledEvent(templates *hfv1.VirtualMachineTemplateList, se *hfv1.ScheduledEvent) error {
	...
	*/
	...
	for envName, vmtMap := range se.Spec.RequiredVirtualMachines {
		...
		env, err := s.hfClientSet.HobbyfarmV1().Environments(util2.GetReleaseNamespace()).Get(s.ctx, envName, metav1.GetOptions{})
		if err != nil {
		...
		if !se.Spec.OnDemand {
			for templateName, count := range vmtMap {
				if count > 0 { // only setup vmsets if >0 VMs are requested, and they aren't ondemand
					...
					existingVMSets, err := s.hfClientSet.HobbyfarmV1().VirtualMachineSets(util2.GetReleaseNamespace()).List(s.ctx, metav1.ListOptions{
						LabelSelector: fmt.Sprintf("%s=%s,%s=%s,virtualmachinetemplate.hobbyfarm.io/%s=true", hflabels.ScheduledEventLabel, se.Name, hflabels.EnvironmentLabel, envName, templateName),
					})
					...
					if err != nil || len(existingVMSets.Items) == 0 { // create new vmset if no existing one was found
						...
						vms := &hfv1.VirtualMachineSet{
							ObjectMeta: metav1.ObjectMeta{
								...
								Labels: map[string]string{
									...
									hflabels.ScheduledEventLabel: se.Name,
						...
						_, err = s.hfClientSet.HobbyfarmV1().VirtualMachineSets(util2.GetReleaseNamespace()).Create(s.ctx, vms, metav1.CreateOptions{})
						if err != nil {
							glog.Error(err)
							return err
						}
						...
						existingVMSet.Labels[hflabels.EnvironmentLabel] = env.Name
						...
						_, err = s.hfClientSet.HobbyfarmV1().VirtualMachineSets(util2.GetReleaseNamespace()).Update(s.ctx, &existingVMSet, metav1.UpdateOptions{})
						if err != nil {
		...
		err = s.hfClientSet.HobbyfarmV1().DynamicBindConfigurations(util2.GetReleaseNamespace()).DeleteCollection(s.ctx, metav1.DeleteOptions{}, metav1.ListOptions{
			LabelSelector: fmt.Sprintf("%s=%s,environment=%s", hflabels.ScheduledEventLabel, se.Name, env.Name),
		})
		if err != nil {
			return err
		}
		...
		_, err = s.hfClientSet.HobbyfarmV1().DynamicBindConfigurations(util2.GetReleaseNamespace()).Create(s.ctx, dbc, metav1.CreateOptions{})
		if err != nil {
	...
	_, err := s.hfClientSet.HobbyfarmV1().AccessCodes(util2.GetReleaseNamespace()).Get(s.ctx, se.Spec.AccessCode, metav1.GetOptions{})
	if err == nil {
...
func (s ScheduledEventController) createAccessCode(se *hfv1.ScheduledEvent) error {
	ac := &hfv1.AccessCode{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				hflabels.ScheduledEventLabel: se.Name,
				hflabels.AccessCodeLabel:     se.Spec.AccessCode,
			},
		},
	...
	ac, err := s.hfClientSet.HobbyfarmV1().AccessCodes(util2.GetReleaseNamespace()).Create(s.ctx, ac, metav1.CreateOptions{})
	if err != nil {
		return err
	}
...
func (s ScheduledEventController) verifyScheduledEvent(se *hfv1.ScheduledEvent) error {
	...
	ac, err := s.hfClientSet.HobbyfarmV1().AccessCodes(util2.GetReleaseNamespace()).Get(s.ctx, se.Spec.AccessCode, metav1.GetOptions{})
	if err != nil {
		err = s.createAccessCode(se)
	...
	} else if ac.Labels[hflabels.AccessCodeLabel] != ac.Spec.Code {
		err = s.deleteAccessCode(se)
		if err != nil {
			return err
		}
...
func (s *ScheduledEventController) reconcileScheduledEvent(seName string) error {
	...
	templates, err := s.hfClientSet.HobbyfarmV1().VirtualMachineTemplates(util2.GetReleaseNamespace()).List(s.ctx, metav1.ListOptions{})
	if err != nil {
		return err
	}
	now := time.Now()
