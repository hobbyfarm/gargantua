...
type VirtualMachineSetController struct {
	...
	vmSetLister      v1.VirtualMachineSetLister
	vmLister         v1.VirtualMachineLister
	envLister        v1.EnvironmentLister
	vmTemplateLister v1.VirtualMachineTemplateLister
...
func (v *VirtualMachineSetController) processNextVMSet() bool {
	...
	err := func() error {
		...
		vmSet, err := v.vmSetLister.VirtualMachineSets(util2.GetReleaseNamespace()).Get(objName)
		if err != nil {
...
func (v *VirtualMachineSetController) reconcileVirtualMachineSet(vmset *hfv1.VirtualMachineSet) error {
	...
	if len(currentVMs) < vmset.Spec.Count { // if desired count is greater than the current provisioned
		...
		env, err := v.envLister.Environments(util2.GetReleaseNamespace()).Get(vmset.Spec.Environment)
		var provision bool
		provision = true
		...
		vmt, err := v.vmTemplateLister.VirtualMachineTemplates(util2.GetReleaseNamespace()).Get(vmset.Spec.VMTemplate)
		if err != nil {
		...
		for i := 0; i < needed; i++ {
			...
			vm, err := v.hfClientSet.HobbyfarmV1().VirtualMachines(util2.GetReleaseNamespace()).Create(v.ctx, vm, metav1.CreateOptions{})
			...
			_, err = v.hfClientSet.HobbyfarmV1().VirtualMachines(util2.GetReleaseNamespace()).UpdateStatus(v.ctx, vm, metav1.UpdateOptions{})

			if err != nil {
				glog.Error(err)
			}

			err = util2.VerifyVM(v.vmLister, vm)
			if err != nil {
				glog.Error(err)
			}
		}
...
func (v *VirtualMachineSetController) updateVMSetCount(vmSetName string, active int, prov int) error {
	retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		result, getErr := v.hfClientSet.HobbyfarmV1().VirtualMachineSets(util2.GetReleaseNamespace()).Get(v.ctx, vmSetName, metav1.GetOptions{})
		if getErr != nil {
		...
		vms, updateErr := v.hfClientSet.HobbyfarmV1().VirtualMachineSets(util2.GetReleaseNamespace()).UpdateStatus(v.ctx, result, metav1.UpdateOptions{})
		if updateErr != nil {
			glog.Error(updateErr)
			return updateErr
		}

		err := util2.VerifyVMSet(v.vmSetLister, vms)
		if err != nil {
			glog.Error(err)
		}
...
func (v *VirtualMachineSetController) removeVMFinalizer(vm *hfv1.VirtualMachine) (err error) {
	if ContainsFinalizer(vm, vmSetFinalizer) {
		...
		_, err = v.hfClientSet.HobbyfarmV1().VirtualMachines(util2.GetReleaseNamespace()).Update(v.ctx, vm, metav1.UpdateOptions{})
	}
	return err
}
