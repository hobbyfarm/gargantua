...
type SessionController struct {
	...
	vmcLister v1.VirtualMachineClaimLister
	ssLister  v1.SessionLister
...
func (s *SessionController) reconcileSession(ssName string) error {
	...
	ss, err := s.ssLister.Sessions(util2.GetReleaseNamespace()).Get(ssName)
	...
	if ss.Status.Finished {
		...
		err = s.hfClientSet.HobbyfarmV1().Sessions(util2.GetReleaseNamespace()).Delete(s.ctx, ss.Name, metav1.DeleteOptions{})
	...
	if expires.Before(now) && !ss.Status.Finished {
		...
		for _, vmc := range ss.Spec.VmClaimSet {
			vmcObj, err := s.vmcLister.VirtualMachineClaims(util2.GetReleaseNamespace()).Get(vmc)
		...
		retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
			result, getErr := s.hfClientSet.HobbyfarmV1().Sessions(util2.GetReleaseNamespace()).Get(s.ctx, ssName, metav1.GetOptions{})
			if getErr != nil {
				return getErr
			}
			...
			result, updateErr := s.hfClientSet.HobbyfarmV1().Sessions(util2.GetReleaseNamespace()).UpdateStatus(s.ctx, result, metav1.UpdateOptions{})
			if updateErr != nil {
				return updateErr
			}
			glog.V(4).Infof("updated result for ss")

			verifyErr := util2.VerifySession(s.ssLister, result)
...
func (s *SessionController) taintVM(vmName string) error {
	...
	retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		result, getErr := s.hfClientSet.HobbyfarmV1().VirtualMachines(util2.GetReleaseNamespace()).Get(s.ctx, vmName, metav1.GetOptions{})
		if getErr != nil {
			return getErr
		}
		result.Status.Tainted = true

		result, updateErr := s.hfClientSet.HobbyfarmV1().VirtualMachines(util2.GetReleaseNamespace()).UpdateStatus(s.ctx, result, metav1.UpdateOptions{})
		if updateErr != nil {
			return updateErr
		}
		glog.V(4).Infof("updated result for vm")

		verifyErr := util2.VerifyVM(s.vmLister, result)
...
func (s *SessionController) taintVMC(vmcName string) error {
	...
	retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
		result, getErr := s.hfClientSet.HobbyfarmV1().VirtualMachineClaims(util2.GetReleaseNamespace()).Get(s.ctx, vmcName, metav1.GetOptions{})
		if getErr != nil {
			return getErr
		}
		result.Status.Tainted = true

		result, updateErr := s.hfClientSet.HobbyfarmV1().VirtualMachineClaims(util2.GetReleaseNamespace()).UpdateStatus(s.ctx, result, metav1.UpdateOptions{})
		if updateErr != nil {
			return updateErr
		}
		verifyErr := util2.VerifyVMClaim(s.vmcLister, result)
		if verifyErr != nil {
			return verifyErr
		}
