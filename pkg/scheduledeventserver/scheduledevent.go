...
func (s ScheduledEventServer) SetupRoutes(r *mux.Router) {
	...
	r.HandleFunc("/a/scheduledevent/new", s.CreateFuncSharedVM).Methods("POST")
	r.HandleFunc("/a/scheduledevent/{id}", s.UpdateFuncSharedVM).Methods("PUT")
...
func (s ScheduledEventServer) CreateFuncSharedVM(w http.ResponseWriter, r *http.Request) {
	user, err := s.auth.AuthGrant(rbacclient.RbacRequest().HobbyfarmPermission(resourcePlural, rbacclient.VerbCreate), w, r)
...
func (s ScheduledEventServer) UpdateFuncSharedVM(w http.ResponseWriter, r *http.Request) {
	_, err := s.auth.AuthGrant(rbacclient.RbacRequest().HobbyfarmPermission(resourcePlural, rbacclient.VerbUpdate), w, r)
	if err != nil {
		util.ReturnHTTPMessage(w, r, 403, "forbidden", "no access to update scheduledevents")
		return
