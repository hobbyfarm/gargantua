...
type ScenarioLister interface {
	...
	List(selector labels.Selector) (ret []*v2.Scenario, err error)
...
// List lists all Scenarios in the indexer.
func (s *scenarioLister) List(selector labels.Selector) (ret []*v2.Scenario, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v2.Scenario))
	})
	return ret, err
}
...
type ScenarioNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v2.Scenario, error)
...
// List lists all Scenarios in the indexer for a given namespace.
func (s scenarioNamespaceLister) List(selector labels.Selector) (ret []*v2.Scenario, err error) {
...
// Get retrieves the Scenario from the indexer for a given namespace and name.
func (s scenarioNamespaceLister) Get(name string) (*v2.Scenario, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v2.Resource("scenario"), name)
	}
	return obj.(*v2.Scenario), nil
}
