/*
Copyright The Kubernetes Authors.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

// Code generated by client-gen. DO NOT EDIT.

package v1

import (
	"time"

	v1 "github.com/hobbyfarm/gargantua/pkg/apis/hobbyfarm.io/v1"
	scheme "github.com/hobbyfarm/gargantua/pkg/client/clientset/versioned/scheme"
	metav1 "k8s.io/apimachinery/pkg/apis/meta/v1"
	types "k8s.io/apimachinery/pkg/types"
	watch "k8s.io/apimachinery/pkg/watch"
	rest "k8s.io/client-go/rest"
)

// ScheduledEventsGetter has a method to return a ScheduledEventInterface.
// A group's client should implement this interface.
type ScheduledEventsGetter interface {
	ScheduledEvents() ScheduledEventInterface
}

// ScheduledEventInterface has methods to work with ScheduledEvent resources.
type ScheduledEventInterface interface {
	Create(*v1.ScheduledEvent) (*v1.ScheduledEvent, error)
	Update(*v1.ScheduledEvent) (*v1.ScheduledEvent, error)
	UpdateStatus(*v1.ScheduledEvent) (*v1.ScheduledEvent, error)
	Delete(name string, options *metav1.DeleteOptions) error
	DeleteCollection(options *metav1.DeleteOptions, listOptions metav1.ListOptions) error
	Get(name string, options metav1.GetOptions) (*v1.ScheduledEvent, error)
	List(opts metav1.ListOptions) (*v1.ScheduledEventList, error)
	Watch(opts metav1.ListOptions) (watch.Interface, error)
	Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *v1.ScheduledEvent, err error)
	ScheduledEventExpansion
}

// scheduledEvents implements ScheduledEventInterface
type scheduledEvents struct {
	client rest.Interface
}

// newScheduledEvents returns a ScheduledEvents
func newScheduledEvents(c *HobbyfarmV1Client) *scheduledEvents {
	return &scheduledEvents{
		client: c.RESTClient(),
	}
}

// Get takes name of the scheduledEvent, and returns the corresponding scheduledEvent object, and an error if there is any.
func (c *scheduledEvents) Get(name string, options metav1.GetOptions) (result *v1.ScheduledEvent, err error) {
	result = &v1.ScheduledEvent{}
	err = c.client.Get().
		Resource("scheduledevents").
		Name(name).
		VersionedParams(&options, scheme.ParameterCodec).
		Do().
		Into(result)
	return
}

// List takes label and field selectors, and returns the list of ScheduledEvents that match those selectors.
func (c *scheduledEvents) List(opts metav1.ListOptions) (result *v1.ScheduledEventList, err error) {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}
	result = &v1.ScheduledEventList{}
	err = c.client.Get().
		Resource("scheduledevents").
		VersionedParams(&opts, scheme.ParameterCodec).
		Timeout(timeout).
		Do().
		Into(result)
	return
}

// Watch returns a watch.Interface that watches the requested scheduledEvents.
func (c *scheduledEvents) Watch(opts metav1.ListOptions) (watch.Interface, error) {
	var timeout time.Duration
	if opts.TimeoutSeconds != nil {
		timeout = time.Duration(*opts.TimeoutSeconds) * time.Second
	}
	opts.Watch = true
	return c.client.Get().
		Resource("scheduledevents").
		VersionedParams(&opts, scheme.ParameterCodec).
		Timeout(timeout).
		Watch()
}

// Create takes the representation of a scheduledEvent and creates it.  Returns the server's representation of the scheduledEvent, and an error, if there is any.
func (c *scheduledEvents) Create(scheduledEvent *v1.ScheduledEvent) (result *v1.ScheduledEvent, err error) {
	result = &v1.ScheduledEvent{}
	err = c.client.Post().
		Resource("scheduledevents").
		Body(scheduledEvent).
		Do().
		Into(result)
	return
}

// Update takes the representation of a scheduledEvent and updates it. Returns the server's representation of the scheduledEvent, and an error, if there is any.
func (c *scheduledEvents) Update(scheduledEvent *v1.ScheduledEvent) (result *v1.ScheduledEvent, err error) {
	result = &v1.ScheduledEvent{}
	err = c.client.Put().
		Resource("scheduledevents").
		Name(scheduledEvent.Name).
		Body(scheduledEvent).
		Do().
		Into(result)
	return
}

// UpdateStatus was generated because the type contains a Status member.
// Add a +genclient:noStatus comment above the type to avoid generating UpdateStatus().

func (c *scheduledEvents) UpdateStatus(scheduledEvent *v1.ScheduledEvent) (result *v1.ScheduledEvent, err error) {
	result = &v1.ScheduledEvent{}
	err = c.client.Put().
		Resource("scheduledevents").
		Name(scheduledEvent.Name).
		SubResource("status").
		Body(scheduledEvent).
		Do().
		Into(result)
	return
}

// Delete takes name of the scheduledEvent and deletes it. Returns an error if one occurs.
func (c *scheduledEvents) Delete(name string, options *metav1.DeleteOptions) error {
	return c.client.Delete().
		Resource("scheduledevents").
		Name(name).
		Body(options).
		Do().
		Error()
}

// DeleteCollection deletes a collection of objects.
func (c *scheduledEvents) DeleteCollection(options *metav1.DeleteOptions, listOptions metav1.ListOptions) error {
	var timeout time.Duration
	if listOptions.TimeoutSeconds != nil {
		timeout = time.Duration(*listOptions.TimeoutSeconds) * time.Second
	}
	return c.client.Delete().
		Resource("scheduledevents").
		VersionedParams(&listOptions, scheme.ParameterCodec).
		Timeout(timeout).
		Body(options).
		Do().
		Error()
}

// Patch applies the patch and returns the patched scheduledEvent.
func (c *scheduledEvents) Patch(name string, pt types.PatchType, data []byte, subresources ...string) (result *v1.ScheduledEvent, err error) {
	result = &v1.ScheduledEvent{}
	err = c.client.Patch(pt).
		Resource("scheduledevents").
		SubResource(subresources...).
		Name(name).
		Body(data).
		Do().
		Into(result)
	return
}
