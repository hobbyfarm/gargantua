	...
	err := func() error {
		...
		for name, vmStruct := range vmClaim.Spec.VirtualMachines {
			if vmStruct.VirtualMachineId == "" {
				...
				if err != nil {
					rollback = true
					glog.Errorf("error while assigning next free VM %v", err)
					break
				}
		...
		if rollback {
			glog.V(6).Infof("rolling back VM assignment")
			for _, vmStruct := range vmClaim.Spec.VirtualMachines {
				if vmStruct.VirtualMachineId != "" {
					err := v.rollbackAssignmentOfVM(vmStruct.VirtualMachineId)
					if err != nil {
						glog.Fatal(err)
					}
				}
			}
			return fmt.Errorf("error while assigning vms to machine")
		}

		// end old logic for binding VM's
		*/
